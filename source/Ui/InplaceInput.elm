module Ui.InplaceInput exposing
  (Model, Msg, init, subscribe, update, view)
...
form model =
  ...
  in
    Ui.Container.column [ onEnter model.ctrlSave Save
                        , onKeys [(27, Close)]
                        ]
      [ Html.App.map Textarea (Ui.Textarea.view model.textarea)
      , Ui.Container.row []
        [ Ui.Button.view Save { disabled = disabled
                              , kind = "primary"
                              , text = "Save"
                              , size = "medium"
                              }
...
display model =
  let
    click = Ui.enabledActions model [onClick Edit]
  in
